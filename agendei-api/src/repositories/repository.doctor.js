...
async function Listar(name){
    ...
    if (name){
         ...
         filtro.push("%" + name + "%") //inserindo name no array
    }
...
async function Excluir(id_doctor){
    ...
    async function ListarServicos(id_doctor){
        ...
        let sql = `select d.id_service,s.description,d.price from doctors_services as d
        inner join services as s on (s.id_service = d.id_service)
        where id_doctor = ?
